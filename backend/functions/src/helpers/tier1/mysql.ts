...
export default class Mysql {
  ...
  static buildSqlQuery(query, tableObject, parentEntries = <Array<string>> [], previousJoins = {}) {
    ...
    for(const entry in query) {
      ...
      if(query[entry]?.fn) {
        ...
        returnObject.select_statement += query[entry].fn((tableObject.alias || tableObject.name) + '.' + entry) + ' AS "' + (parentEntriesCopy.length > 0 ? parentEntriesCopy.join('.') + '.' : '') + entry + '"';
      } else if(query[entry]?.__typename && query[entry]?.mysqlOptions?.joinInfo) {
        //process __typename fields
        //check if it is a joinable field and assemble the join statement
        const joinTableObject = {
          name: query[entry].__typename,
          alias: null
        };
  ...
  static processGraphqlWhereArray(table, whereArray, previous_joins, params) {
    ...
    whereArray.forEach((whereObject, whereIndex) => {
      ...
      for(const fieldname in whereObject.fields) {
        ...
        const joinTableName = (typeDef[fieldname]?.mysqlOptions.joinInfo && fieldvalue.foreignField) ? typeDef[fieldname].__typename : null;
  ...
  static getPool = mysql.getPool;
  
  static getSequelizeInstance = mysql.getSequelizeInstance;

  static Sequelize = mysql.Sequelize;
